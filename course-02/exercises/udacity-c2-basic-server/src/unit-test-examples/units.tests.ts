...
describe('concat', () => {
    ...
    const kit = 'kit';
    const ten = 'ten';
    it(`should concat ${kit} and ${ten}`, () => {
        const result = concat(kit, ten);
        expect(result).to.equal('kitten');
    });

    const joseph = 'Joseph ';
    const stalin = 'Stalin';
    it(`should concat ${joseph} and ${stalin}`, () => {
        const result = concat(joseph, stalin);
        expect(result).to.equal("Joseph Stalin");
    });
